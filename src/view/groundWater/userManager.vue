<template>
  <div class="div-border">
    ...
    <Form ref="search" :model="search" :label-width="80">
      <Row>
        <Col span="8">
          <FormItem label="操作员" prop="userName">
            <Input type="text" v-model="search.userName" placeholder="请输入想要查询的操作员名" />
          </FormItem>
        </Col>
        <Col span="8">
    ...
    <!-- 操作按钮 -->
    <Button type="info" icon="ios-add-circle-outline" class="opBtn" @click="showAddModal">新增操作员</Button>
    ...
    <Modal
      ...
      <Form ref="add" :model="add" :rules="addValidate" :label-width="100">
        <Row>
          <Col span="24">
            <FormItem label="操作员" prop="userName">
              <Input v-model="add.userName" placeholder="请输入操作员"></Input>
            </FormItem>
          </Col>
        </Row>
        <Row>
    ...
    <Modal
      ...
      <Form ref="update" :model="update" :rules="updateValidate" :label-width="100">
        <Row>
          <Col span="24">
            <FormItem label="操作员" prop="userName">
              <Input disabled v-model="update.userName" placeholder="请输入用操作员"></Input>
            </FormItem>
          </Col>
        </Row>
        <Row>
...
export default {
      ...
      data: [
        {
          ...
          department: '部门',
          contact: '',
          createTime: '20180606102600',
          lastLoginTime: '20190930133600',
          lastPwdTime: '20180606102600',
          pwdValidity: 0,
          state: '1'
        }
      ],
  ...
  methods: {
    ...
    deleteUser (user) {
      ...
      this.$Modal.confirm({
        title: '操作确认',
        content: '确认删除用户[' + user + ']吗？',
        onOk: function () {
          this.axios.delete('/role/' + user).then(resp => {
            if (resp.code === 200) {
